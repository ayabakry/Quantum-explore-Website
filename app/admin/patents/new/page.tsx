...
export default function NewPatent() {
  ...
  const addInventor = () => {
    setFormData((prev) => ({ ...prev, inventors: [...prev.inventors, ""] }))
  }
  ...
  const validateForm = () => {
    ...
    if (!formData.id.trim()) {
      newErrors.id = "Patent ID is required"
    }
    ...
    if (formData.status === "Granted" && !formData.issueDate.trim()) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    toast({
      title: "Patent added",
      description: "The patent has been successfully added.",
    })
